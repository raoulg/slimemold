...
to setup-nodes
  ...
  let x-range n-values (max-pxcor - node-diameter / 2) [ ?1 -> ?1 + 1 ]
  let y-range n-values (max-pycor - node-diameter / 2) [ ?1 -> ?1 + 1 ]
        ...
        foreach remaining-nodes [ ?1 ->
          create-edge-with ?1 [
           ; hide-turtle
            ...
            set node-a a
            set node-b ?1
            set cost ceiling calculate-distance a ?1
...
to-report choose-next-node [current-node remaining-nodes]
  ...
  report last first filter [ ?1 -> first ?1 >= rand-num ] probabilities
end
...
to-report calculate-probabilities [current-node remaining-nodes]
  ...
  foreach remaining-nodes [ ?1 ->
    ask current-node [
      let next-edge edge-with ?1
      let transition-probability ([pheromone] of next-edge ^ alpha) * ((1 / [cost] of next-edge) ^ beta)
      set transition-probabilities lput (list transition-probability ?1) transition-probabilities
      set denominator (denominator + transition-probability)
    ]
  ]

  let probabilities []
  foreach transition-probabilities [ ?1 ->
    let transition-probability first ?1
    let destination-node last ?1
  ...
  let total 0
  foreach probabilities [ ?1 ->
    set total (total + first ?1)
    set normalized-probabilities lput (list total last ?1) normalized-probabilities
  ]
...
to update-pheromone
  ...
  ask ants [
    without-interruption [
      ...
      foreach get-tour-edges tour [ ?1 ->
        ask ?1 [ set pheromone (pheromone + pheromone-increment) ]
      ]
    ]
  ]
end
...
to update-best-tour
  ...
  foreach get-tour-edges best-tour [ ?1 ->
    ask ?1 [ set hidden? false ]
  ]
end
...
to-report get-tour-length [tour-nodes]
  report reduce [ [?1 ?2] -> ?1 + ?2 ] map [ ?1 -> [cost] of ?1 ] get-tour-edges tour-nodes
end
...
This model is an implementation of the Ant System algorithm, as described by Marco Dorigo.  It tries to solve the Traveling Salesman Problem.
